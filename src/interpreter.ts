...
export async function evaluate(exprs: Expr[], env: Environment = new Env()): Promise<Value> {
  ...
  for (const expr of exprs) {
    ...
    if (result.kind === 'return') break; // stop on return at top-level
  }
  return result;
}
  ...
  if (fn.name === 'startServer') {
    ...
    serverInstance = http.createServer(async (req, res) => {
      ...
      if (matchedRoute) {
        ...
        // Safety: ensure handler is a function
        if (handler.kind !== 'function') {
          res.writeHead(500);
          res.end('Internal Server Error: handler is not a function');
          return;
        }
...
function stringify(val: Value): string {
  switch (val.kind) {
    ...
    case 'return': return `<return ${stringify(val.value)}>`;
